...
public class Thread {
	...
	public void start() throws Exception {
		while(!jvmStack.isEmpty()) {
			...
			ConstantPool constantPool = currentFrame.constantPool();
			Instruction currentInstruction = currentFrame.getNextInstruction();
			switch (currentInstruction.getOpcode()) {
				case getstatic -> {
					...
					if(className.contains("java")) {
						...
						currentFrame.pushObjectToOperandStack(staticField);
					}
				}
				...
				case iload_0 -> jvmStack.peek().pushObjectToOperandStack(jvmStack.peek().localVariable.get(0));
				case iload_1 -> jvmStack.peek().pushObjectToOperandStack(jvmStack.peek().localVariable.get(1));
				...
				case invokevirtual -> {
					...
					if(className.contains("java")) {
						...
						for(int i = params.size() - 1;i>=0;i--) {
							args[i] = currentFrame.operandStack.pop();
						}
						// 第一个参数是对象本身 存在操作数栈顶，为啥？？ 因为调用方法的时候，是先把对象引用压入操作数栈，然后才是参数，什么时候压进去的？？？
						Object result = declaredMethod.invoke(currentFrame.operandStack.pop(),args);
						if(!methodInfo.isVoid(constantPool)) {
							currentFrame.pushObjectToOperandStack(result);
						}
						break;
					}
